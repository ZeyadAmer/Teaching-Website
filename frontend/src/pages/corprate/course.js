...
export default function Course() {
    ...
    if (courses != null) {
        ...
        subtitles = courseView.subtitles.map((subtitle) =>
            < Subtitle key={subtitle._id} subtitle={subtitle} onClick={() => window.location.href = `/corprate/subtitle?subtitleId=${subtitle._id}&courseId=${courseId}`} />
        )
        excersices = courseView.Excercises.map((excersice) =>
            < Excersice key={excersice._id} excersice={excersice} onClick={() => window.location.href = `/corprate/excersice?excersiceId=${excersice._id}&courseId=${courseId}`} />
        )
    ...
    return (
        <div>
            ...
            {excersices}
            <div className='search'>
                <button onClick={() => window.location.href = `/corprate/report?courseId=${courseId}`}>Report a Problem</button>
            </div>
        </div>
    )
}
